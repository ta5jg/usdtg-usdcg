...
const abi = JSON.parse(fs.readFileSync('./build/contracts/FlashTetherTRC20.json')).abi;
...
async function main() {
  ...
  console.log(`📈 1 USDTz'nin USD Fiyatı: ${price.toString()} (8 decimals)`);
  console.log(`💵 1 USDTz = $${(usdValue / 1e8).toFixed(6)}`);
  console.log(`👛 Cüzdan Bakiyesi: ${balance} (${(balance / 1e6).toLocaleString()} USDTz)`);
